...
#
#   python repy.py restrictions.test dump.py [optional: filename]
#
#   Chord Legend:
#       SHA-1               Ln 29
#       Chord Node          Ln 355
...
#       Remote Node         Ln 600
#       Local Node          Ln 916
#       Communication Fnc   Ln 1051
#       Synchronization Fnc Ln 1290
#       proc2               Ln 1366
#       Verify Overlay      Ln 1449
#       main                Ln 1557
#
#
#
...
class VerifyOverlay:
    def __init__(self,filename='nodes.conf'):
        self.nodes = self.LoadNodes(filename)
        self.PrintNodes(self.nodes)
        self.VerifyNodes(self.nodes)
    ...
    def LoadNodes(self, filename):
        ...
        try:
            ...
            for i in addresses:
                if i:
                    add = i.split(':')
                    n = RemoteNode(add[0], int(add[1])).dumpself()
    ...
    def PrintNodes(self, nodes, printdata=True):
        #do a clean print
        list_k = list(nodes)
        bubbleSort(list_k)
        print 'Printing sorted chord ring:'
        print "predecessor <- node -> successor"
        #for n in nodes.values():
        for idx in range(len(list_k)):
            n = nodes[list_k[idx]]
            succid = '  '
            predid = '  '
            ...
            if n.successor is not None:
                succid = n.successor.id
            
            print predid,"<-",n.id,"->",succid,'\t',n.ip,n.port

        if printdata:
            print 'Printing node key/values:'
            for idx in range(len(list_k)):
                print 'node:', nodes[list_k[idx]].id, 'data:', nodes[list_k[idx]].data

        print 'Printing node fingers:'
        #for n in nodes.values():
        for idx in range(len(list_k)):
            n = nodes[list_k[idx]]
            print 'The fingers for node', n.id, 'are:'
            for i in range(0,len(n.finger)):
                if n.finger[i] is None:
                    print '\tfinger:', i, 'is', n.finger[i]
                else:
                    print '\tfinger:', i, 'is', n.finger[i].id
                
    def VerifyNodes(self, nodes):
        #sort the nodes
        if len(nodes) == 1:
            print 'Only one node, nothing to verify'
            return
        ...
        for idx in range(len(list_k)):
            ...
            print 'verifying node', nodes[list_k[idx]].id, nodes[list_k[idx]].ip, nodes[list_k[idx]].port 
            #check if predecessor and successor are correct
            if nodes[list_k[idx]].predecessor is not None:
                if nodes[list_k[idx]].predecessor.id != list_k[p_idx]:
                    print '\tnode has predecessor', nodes[list_k[idx]].predecessor.id, 'but it should be', list_k[p_idx]
                else:
                    print '\tnode has a correct predecessor'
            else:
                print '\tnode is missing predecessor'
                
            if nodes[list_k[idx]].successor is not None:
                if nodes[list_k[idx]].successor.id != list_k[s_idx]:
                    print '\tnode has successor', nodes[list_k[idx]].successor.id, ' but it should be', list_k[s_idx]
                else:
                    print '\tnode has a correct successor'
            else:
                print '\tnode is missing successor'
    
if callfunc == 'initialize':
    mycontext['traceLevel'] = 1
