...
#   put.py
#
#   Command-Line Arguments: ip port key value
#
#   python repy.py restrictions.test put.py ip port key value
#
#   Chord Legend:
#       SHA-1               Ln 29
#       Chord Node          Ln 355
...
if callfunc == 'initialize':
    ...
    #input: ip port key value
    port = 12345#12359#
    ip = getmyip()#'127.0.0.1'#

    if len(callargs) != 4:
        print 'Please enter: ip port key value'
    else:
        remoteip = callargs[0]
        remoteport = int(callargs[1])
        myid = int(callargs[2])
        myval = callargs[3]
        ...
        n = RemoteNode(remoteip, remoteport)
        
        n0 = n.find_successor(myid)

        if n0 is not None:
            print 'Adding value to node', n0.id, n0.ip, n0.port
            ret = n0.put(myid, myval)
            print '************************************************'
            if 'ack' in ret[0] and ret[0]['ack'] == 'ok':
                print 'Added Value Successfully'
            elif 'ack' in ret[0] and 'err' in ret[0]:
                print 'Unable to add Value. Error:',  ret[0]['err']
            else:
